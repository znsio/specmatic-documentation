...
See [Reference](./reference#custom-rule-node-types) for supported node types.
...
If loading fails with `unknown node type` or `Unsupported assertion`, check [custom-rule assertions](./reference#custom-rule-assertions) and [node types](./reference#custom-rule-node-types).
