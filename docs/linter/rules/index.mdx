...
Start with [`no-schema-type-mismatch`](/linter/rules/schema/no-schema-type-mismatch).

:::note
This reference explains rule behavior. To see rules enabled by your configuration, generate the [effective rule catalog](/linter/built-in-rules#generate-the-rule-catalog).
:::
