...
Transient examples (`"transient": true` or `"http-stub-id"`) and persistent examples are checked in separate buckets. A transient example that overlaps a persistent example is not competing, because the mock uses the transient example first and can still fall back to the persistent example afterward. Two overlapping transient examples, or two overlapping persistent examples, are still reported as competing.
