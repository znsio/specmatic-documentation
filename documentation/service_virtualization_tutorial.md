...
- [Service Virtualization](#service-virtualization)
  ...
  - [Add External Examples](#add-external-examples)
  ...
  - [Examples matching any value matching the specification](#examples-matching-any-value-matching-the-specification)
  - [Values matching regular expressions](#values-matching-regular-expressions)
  ...
  - [Precedence across examples](#precedence-across-examples)
...
- [Download the latest version of Specmatic](/download.html) into the `specmatic` directory.
- Install JRE 17.
- `cd` into the `specmatic` directory and run the command `java -jar specmatic.jar --version`. This should print out the version of Specmatic.

## Inline Examples

Create the following files in the `specmatic` directory.
...
- In the same directory, create a file named `specmatic.yaml` in the `specmatic` directory with the following contents:
...
In the specification above, the request and response both contain examples with the same name, `CREATE_EMPLOYEE_SUCCESS`. Specmatic stub correlates the two, and returns value in the response example when it the value in the request example is received
...
- You'll see it writing out a file into the `employees_examples` directory
  - The directory name MUST follow the format `<spec file name without extension>_examples`.
  - The file name indicates the operation it was generated from, but is free-form and need follow no convention.
...
- This is an external example. It is loaded by Specmatic automatically when the stub starts.
- In the request, modify the value of `name` to "Jack Sprat", and the value of `department` to "Sales", and in the response, modify the value of `id` to 20, like so:
...
What happens if an example goes out of sync with the spec? Let's see.

- Copy the example JSON file to a new file in the `_examples` directory.
- Modify the value of `name` in the request to "Janet", and the value of `id` in the response to "abc123" (remember, `id` is an integer according to the specification), like so:
...
- It's a mistake that anyone can make, especially in more complicated APIs.
- Start the stub.
- You'll see the following error message in the log:
...
- In this way, Specmatic give early feedback whenever eamples are out of line with the spec.


- Specmatic's stub rejected the example because of the mismatch. Prove this by running the following curl command:
...
- Specmatic stub had rejected the example, but the request does match the specification. So a structure matching the specification with random values is generated and returned.

## Examples matching any value matching the specification
...
- Execute the following command, which has the `name` "Janet":
...
- Modify the value of `department` to "Facilities", and you'll get a randomised response.
- This example will match a request with any name, but the value of `department` in the request must be "Engineering", and the value of `designation` must be "Director".

## Values matching regular expressions

For specifications where the request body is a string with some custom format, you can use regular expressions in the example.

Let's use a new specification named employee_sql.yaml:
...
- Create a directory named `employee_sql_examples` and insite that create a file named `select_employee_by_id.json`:
- Create a new file in the `employees_examples` directory named `names_starting_with_j.json`:
...
- Try changing the table to anything else. Just make sure that the query starts with `SELECT` and ends with `id = 1`, as per the regex in `bodyRegex` in the example, and you'll get the same response.

## Correlated request and response values

Response values can be correlated with request values in the following ways.

Update employees.yaml specification with the following content:
...
If the multiple values in the response depends on the name, and they are all different, the data lookup approach would come in handy.
  ...
  ```json
  {
      ...
      "http-response": {
          ...
          "body": {
              ...
              "designation": "$(dataLookup.departments[DEPARTMENT].designation)"
          }
      },
      "dataLookup": {
          "departments": {
...
- Look closely at the syntax of the value of `designation` in the response.
  - Explanation: The value of `designation` will be arrived at by looking up in the `dataLookup.departments`, dereferencing the dictionary based on `DEPARTMENT` (which came from the value of `department` in the response), and in the resulting dictionary, the value of `designation` is used.
  - Note that the names `dataLookup`, `departments`, `designation` are all free-form. There are no conventions to be followed.
...
- The value of `designation` in the response varies based on the value of `department` in tihe request.

## Partial examples

If you are only interested in matching certain values in the request, you can setup a partial example.

For example, suppose I want to setup a response for all requests with `name` set to "George". I don't care about the others, let them be what they may.

- Create a file in `employees_examples` named partial_request.json, with the following contents:
