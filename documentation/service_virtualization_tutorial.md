...
- Execute the following `curl` command:
  ...
  ```shell
  curl -X PATCH -H 'Content-Type: application/json' -d '{"name": "Janet"}' http://localhost:9000/employees
  ```
...
Try the same curl command again but modify the name. You'll see that the name sent in the request will be reflected in the response.
...
```shell
curl -X PATCH -H 'Content-Type: application/json' -d '{"department": "Sales"}' http://localhost:9000/employees
```
