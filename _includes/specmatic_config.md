...
* **exposes** - This is the list of API Specifications that need to be run as a test. Note that the path is relative to the source control repository root.
...
Now if you run the ```specmatic test``` command line executable from the directory that contains the ```specmatic.json``` file Specmatic will pull the API Specifications listed under exposes and run them as tests.
...
A single application may need to list the API Specifications it is implementing under the exposes attribute and the API Specifications of its dependencies under the dependencies attribute.
...
Note that the `dependencies` and `exposes` specifications are relative paths. This means that they must be in the same directory as the current directory.
...
Note that the `dependencies` and `exposes` can both contain URLs. `http` and `https` are both supported.
